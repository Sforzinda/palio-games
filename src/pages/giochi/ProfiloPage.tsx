...
export default function ProfiloPage() {
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: 'linear-gradient(180deg, #f7ecd3 0%, #efe2c2 100%)' }}>
      <Header />
      <main className="flex-1 px-4 py-8">
        <div className="max-w-4xl mx-auto">
          <div className="rounded-2xl border-4 border-amber-700 p-6 shadow-2xl" style={{ background: 'radial-gradient(circle at top, #fff8ea 0%, #f1dfbf 100%)' }}>
            <div className="flex justify-between mb-1">
              <Link to="/giochi" className="inline-flex items-center gap-1 text-sm font-semibold text-palio-700 hover:text-palio-900 underline underline-offset-2">
                ← Torna ai giochi
              </Link>
              <Link to="/giochi/classifica" className="inline-flex items-center gap-1 text-sm font-semibold text-palio-700 hover:text-palio-900 underline underline-offset-2">
                Classifica →
              </Link>
            </div>
            <p className="text-center text-amber-800 text-lg">⚜ 🛡 ⚜</p>
            <h1 className="font-medieval text-4xl text-center text-palio-900 mt-2">Profilo del Giocatore</h1>
            <p className="text-center text-amber-900/80 mt-2">Le tue onorificenze nei Giochi del Palio</p>

            {loading && (
              <div className="py-14 text-center">
                <div className="inline-block h-9 w-9 rounded-full border-4 border-amber-600 border-t-transparent animate-spin" />
                <p className="mt-3 text-amber-900">Caricamento profilo...</p>
              </div>
            )}

            {!loading && error && (
              <div className="mt-6 rounded-xl border-2 border-amber-700 bg-amber-50 p-5 text-center text-amber-900">
                <p>{error}</p>
                <div className="mt-4 flex flex-wrap justify-center gap-3">
                  <Link to="/giochi" className="btn-game inline-block">Vai ai giochi</Link>
                  <button
            ...
            {!loading && !profile && !error && (
              <div className="mt-6 rounded-xl border-2 border-amber-700 bg-[#fff8e9] p-6">
                <h2 className="font-medieval text-xl text-palio-900 text-center">Accedi al tuo profilo</h2>
                <p className="mt-1 text-sm text-amber-900/70 text-center">
                ...
                <p className="mt-4 text-center text-sm text-amber-900/60">
                  Non hai un account?{' '}
                  <Link to="/giochi" className="font-semibold underline underline-offset-2 text-palio-700 hover:text-palio-900">
                    Gioca e registrati
                  </Link>
                </p>
              </div>
            ...
            {!loading && !error && profile && (
              <>
                ...
                {saveSuccess && (
                  <div className="mt-4 rounded-xl border border-emerald-700 bg-emerald-50 px-4 py-3 text-sm text-emerald-900">
                    {saveSuccess}
                  </div>
                )}
                <div className="mt-3 grid gap-4 sm:grid-cols-4">
                  <div className="rounded-xl border-2 border-amber-700/60 bg-[#fff8e9] p-4 text-center">
                    <p className="text-xs uppercase tracking-wide text-amber-800">Giocatore</p>
                    <p className="mt-2 text-lg font-semibold text-palio-900">{profile.name}</p>
                  </div>
                  <div className="rounded-xl border-2 border-amber-700/60 bg-[#fff8e9] p-4 text-center">
                    <p className="text-xs uppercase tracking-wide text-amber-800">Citta</p>
                    <p className="mt-2 text-lg font-semibold text-palio-900">{profile.city}</p>
                  </div>
                  <div className="rounded-xl border-2 border-amber-700/60 bg-[#fff8e9] p-4 text-center">
                    <p className="text-xs uppercase tracking-wide text-amber-800">Contrada</p>
                    <p className="mt-2 text-base font-semibold text-palio-900">{profile.contradaName || 'Non impostata'}</p>
                  </div>
                  <div className="rounded-xl border-2 border-amber-700/60 bg-[#fff8e9] p-4 text-center">
                    <p className="text-xs uppercase tracking-wide text-amber-800">Partite giocate</p>
                    <p className="mt-2 text-2xl font-bold text-palio-900">{profile.totalGamesPlayed}</p>
                  </div>
                </div>
                ...
                {editMode && (
                  <form
                    ...
                    className="mt-6 rounded-xl border-2 border-amber-700/60 bg-[#fff8e9] p-5"
                  >
                    <h2 className="font-medieval text-2xl text-palio-900">Modifica profilo</h2>
                ...
                {profile.gameStats.length > 0 && (
                  <div className="mt-8">
                    <h2 className="font-medieval text-2xl text-palio-900">Statistiche per gioco</h2>
                    <div className="mt-4 grid gap-3 sm:grid-cols-2">
                      {profile.gameStats.map((stat) => (
                        <div
                          key={stat.gameId}
                          className="rounded-xl border-2 border-amber-700/60 bg-[#fff8e9] p-4"
                        >
                          <p className="font-medieval text-base capitalize text-palio-900">{stat.gameId}</p>
                          <div className="mt-2 flex gap-6 text-sm text-amber-900">
                ...
                <div className="mt-8">
                  <h2 className="font-medieval text-2xl text-palio-900">Badge conquistati</h2>
                  {badges.length === 0 ? (
                    <div className="mt-4 rounded-xl border-2 border-amber-700/40 bg-[#fff8e9] p-6 text-amber-900">
                    ...
                    <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                      {badges.map((badge) => (
                        <article
                          ...
                          className="rounded-xl border-2 border-amber-700 bg-[#fff6e0] p-4 shadow-lg"
                          style={{ boxShadow: 'inset 0 0 0 1px #f0d07a, 0 8px 24px rgba(92, 45, 6, 0.18)' }}
                        >
                          <div className="flex items-center gap-3">
                            <GameBadgeMedallion badge={badge} />
                            <div>
                              <h3 className="font-medieval text-lg text-palio-900">{badge.name}</h3>
                              <p className="text-xs text-amber-900/70">🛡 Emblema del Ducato</p>
                            </div>
                          </div>
                          <p className="mt-3 text-sm text-amber-950">{badge.description}</p>
